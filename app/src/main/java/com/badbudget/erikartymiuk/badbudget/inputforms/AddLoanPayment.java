...
/**
 * Add Loan Payment Activity - activity displays a form for setting up a payment for a debt. The
 * debt information is filled in with data already taken from the add loan form,
 * but can still be edited. Heavily influenced by the Add Savings Activity
 *
 * This form is also used to edit an existing loan to either add a new payment or edit an
 * existing one.
 */
public class AddLoanPayment extends BadBudgetChildActivity
        implements AdapterView.OnItemSelectedListener, DateInputActivity,
        CleanFragmentParent, BBOperationTaskCaller
{
    private static final String progressDialogMessage = "Adding Loan With Payment...";
    private static final String progressDialogMessageUpdate = "Updating Loan With Payment...";
    private static final String progressDialogMessageDelete = "Deleting Loan With Payment...";
    ...
    * where items are matched with keys in clean button method */
    ...
    Toast alreadyCleanToast;
    Toast missingCleanDataToast;
    Toast submitErrorToast;
    Toast calculateGoalErrorToast;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContent(R.layout.content_add_loan_payment);
    ...
    private void setup()
    {
        debtName = (EditText) findViewById(R.id.addLoanPaymentDebtHint);
        debtAmount = (EditText) findViewById(R.id.addLoanPaymentCurrentValueHint);
        interestRateInput = (EditText) findViewById(R.id.addLoanPaymentInterestRateHint);
        ...
        quicklookCheckbox = (CheckBox) findViewById(R.id.addLoanPaymentQuicklookCheckbox);

        paymentAmount = (EditText) findViewById(R.id.addLoanPaymentAmountHint);
        payoffCheckbox = (CheckBox) findViewById(R.id.addLoanPaymentAmountPayoff);

        paymentFrequencySpinner = (Spinner) findViewById(R.id.addLoanPaymentFrequencySpinner);
        paymentSourceSpinner = (Spinner) findViewById(R.id.addLoanPaymentSourceSpinner);

        nextDateText = (TextView) findViewById(R.id.addLoanPaymentNextInput);

        endDateText = (TextView) findViewById(R.id.addLoanPaymentInputEnd);
        ongoingCheckbox = (CheckBox) findViewById(R.id.addLoanPaymentOngoingCheckbox);

        setGoalCheckbox = (CheckBox) findViewById(R.id.addLoanPaymentSetGoal);
        goalDateText = (TextView) findViewById(R.id.addLoanPaymentInputGoal);
        ...
        * to reference the accounts by using the name as a key */
        ...
        alreadyCleanToast = Toast.makeText(this, R.string.add_loan_payment_data_clean_message, Toast.LENGTH_SHORT);
        ...
        editLoan = (Loan)((BadBudgetApplication)this.getApplication()).getBadBudgetUserData().getDebtWithName(editName);
        editHasExistingPayment = args.getBoolean(BadBudgetApplication.EDIT_PAYMENT_KEY);

        if (editing)
        {
            this.setToolbarTitle(R.string.edit_loan_payment_title);
            debtName.setEnabled(false);
            debtName.setText(editLoan.name());
    ...
    private void firstTimeSetup()
    {
        ...
        {
            //Fill out the basic debt info for the existing loan with a payment
            //We came directly from the debts table
            debtAmount.setText(Double.toString(editLoan.amount()));
            ...
            Payment payment = editLoan.payment();
        ...
        else
        {
            ...
            interestRateInput.setText(Double.toString(args.getDouble(BadBudgetApplication.INPUT_DEBT_INTEREST_RATE_KEY)));

            boolean simpleInterest = args.getBoolean(BadBudgetApplication.INPUT_LOAN_SIMPLE_INTEREST_KEY);
            if (simpleInterest)
            {
                simpleInterestCheckbox.setChecked(true);
                simpleInterestCheckboxChecked();
                principalInput.setText(Double.toString(args.getDouble(BadBudgetApplication.INPUT_LOAN_PRINCIPAL_KEY)));
            }
            else
            {
                simpleInterestCheckbox.setChecked(false);
    ...
    * when setting up the form for the first time) */
    ...
    private void payoffChecked()
    {
        ...
        paymentAmount.setHint(R.string.add_loan_payment_amount_payoff);
    }
    ...
    private void payoffUnchecked()
    {
        ...
        paymentAmount.setHint(R.string.add_loan_payment_amount_hint);
    }
    ...
    private void ongoingChecked()
    {
        ...
        clearEndDate();
        endDateText.setHint(getString(R.string.add_loan_payment_ongoing));
    }
    ...
    private void ongoingUnchecked()
    {
        ...
        endDateText.setHint(R.string.add_loan_payment_date_hint);
    }
    ...
    private void disableEndDateInput()
    {
        ...
        endDateText.setClickable(false);
        clearEndDate();
        endDateText.setHint(R.string.add_loan_payment_end_date_disabled);
    }
    ...
    {
        dateClicked(AddLoanPayment.NEXT_DATE_SET_CODE, "nextDatePicker");
    }
    ...
    {
        dateClicked(AddLoanPayment.END_DATE_SET_CODE, "endDatePicker");
    }
    ...
    {
        dateClicked(AddLoanPayment.GOAL_DATE_SET_CODE, "goalDatePicker");
    }
     ...
     * @param code - the code identifying which date is being chosen (constants in AddLoanPaymentActivity)
     ...
     * @param view
     */
    public void addLoanPaymentCleanClick(View view)
    {
        ...
        //Simple interest must either be unchecked or checked with the principal amount entered
        boolean simpleInterestGood = !simpleInterestCheckbox.isChecked() || (simpleInterestCheckbox.isChecked()
                && !principalInput.getText().toString().equals(""));
        ...
        boolean allEntered = currentValueEntered && interestRateEntered && simpleInterestGood && paymentAmountEntered && goalCheckboxChecked && nextDateEntered && goalSet;

        //This is not ok. User needs to input more data
        boolean missingKey = !currentValueEntered || !interestRateEntered || !simpleInterestGood || !nextDateEntered || !goalCheckboxChecked;
        ...
        if (allEntered)
        {
            ...
            //TODO 3/15/2017 - once find payment simple interest is implemented check this
            ...
            if (simpleInterestCheckbox.isChecked())
            {
                dataConsistent = Prediction.datesEqualUpToDay(currentGoalDate, correctGoalDate); //|| paymentAmountGood;
            }
            else
            {
                dataConsistent = Prediction.datesEqualUpToDay(currentGoalDate, correctGoalDate) || paymentAmountGood;
            ...
            if (!dataConsistent)
            {
                ...
                bundle.putString(CleanFragment.POSITION_ONE_TITLE_KEY, getString(R.string.add_loan_payment_goal_date_prompt));
                ...
                bundle.putString(CleanFragment.POSITION_TWO_TITLE_KEY, getString(R.string.add_loan_payment_amount_prompt));
        ...
        else if (missingOnlyPaymentAmount)
        {
            ...
            CharSequence text = getString(R.string.add_loan_payment_auto_set_payment_amount);
        ...
        else if (missingOnlyGoalDate)
        {
            ...
            CharSequence text = getString(R.string.add_loan_payment_auto_set_goal_date);
        ...
        {
            String goalCheckboxUnchecked = getString(R.string.add_loan_payment_data_goal_checkbox_unchecked);
        ...
        else
        {
            ...
            String missingMessage = getString(R.string.add_loan_payment_data_clean_missing);
            if (missingPaymentAndGoal)
            {
                missingMessage+="\n";
                missingMessage+= (getString(R.string.add_loan_payment_amount_prompt) + " " +
                        getString(R.string.add_loan_payment_data_clean_and_or) + " " +
                        getString(R.string.add_loan_payment_goal_date_prompt));
            }
            if (!currentValueEntered)
            {
                missingMessage+="\n";
                missingMessage+=getString(R.string.add_loan_payment_value_prompt);
            }
            if (!interestRateEntered)
            {
                missingMessage+="\n";
                missingMessage+=getString(R.string.add_loan_payment_interest_rate_prompt);
            }
            if (!simpleInterestGood)
            {
                missingMessage+="\n";
                missingMessage+=getString(R.string.add_loan_simple_interest_principal);
            }

            if (!nextDateEntered)
            {
                missingMessage+="\n";
                missingMessage+=getString(R.string.add_loan_payment_next_prompt);
            }
     ...
     * a new loan object with a payment.
     * @param view
     */
    public void addLoanPaymentSubmitClick(View view)
    {
        if (verifyValues())
        {
            ...
            try
            {
                ...
                Payment payment = new Payment(pAmount, payoff, pFreq, pSource, pNextDate, pOngoing, pEndDate, loan, pGoalDate);
                ...
                values.put(BBDatabaseContract.Debts.COLUMN_SIMPLE_INTEREST, simpleInterest);
                ...
                if (editing)
                {
                    EditBBObjectTask task = new EditBBObjectTask(this, progressDialogMessage, loan, values, this, BBObjectType.DEBT_LOAN);
                    task.execute();
                }
                else
                {
                    AddBBObjectTask task = new AddBBObjectTask(this, progressDialogMessageUpdate, loan, values, this, BBObjectType.DEBT_LOAN);
                    task.execute();
                }
            }
     ...
     * @param view
     */
    public void addLoanPaymentCancelClick(View view)
    {
        this.onBackPressed();
    }
     ...
     * Kicks off a task to delete the Loan we are editing from our bb data.
     * @param view
     */
    public void deleteClick(View view)
    {
        DeleteBBObjectTask task = new DeleteBBObjectTask(this, progressDialogMessageDelete, editLoan.name(), this, BBObjectType.DEBT_LOAN);
        task.execute();
    }
    ...
    private Double calculatePaymentAmount()
    {
        ...
        Double correctPaymentAmount;
        if (simpleInterestCheckbox.isChecked())
        {
            Double principal = Double.parseDouble(this.principalInput.getText().toString());
            //TODO 3/15/2017 - unimplemented
            //correctPaymentAmount = Prediction.findPaymentAmountSimpleInterest(nextDate, numInterestDays, goalDate, paymentFrequency, currentAmount, interestRate, principal);
            correctPaymentAmount = 0.0;
        }
        else
        {
            correctPaymentAmount = Prediction.findPaymentAmountCompoundInterest(nextDate, numInterestDays, goalDate, paymentFrequency, currentAmount, interestRate);
        }
     ...
     * a goal date. Returns a goal date consistent with all the values currently set or null if one cannot be found.
     * @return - a consistent goal date, or null if a consistent goal date cannot be found within our span (R.integer.predict_years)
     */
    private Date calculateGoalDate()
    {
        ...
        Date correctGoalDate;
        if (simpleInterestCheckbox.isChecked())
        {
            Double principal = Double.parseDouble(this.principalInput.getText().toString());
            correctGoalDate = Prediction.findGoalDateSimpleInterest(nextDate, numInterestDays, paymentAmount, paymentFrequency, currentAmount, interestRate, principal, limitDate);
        }
        else
        {
            correctGoalDate = Prediction.findGoalDateCompoundInterest(nextDate, numInterestDays, paymentAmount, paymentFrequency, currentAmount, interestRate, limitDate);
        }
    ...
    private boolean verifyValues()
    {
        ...
        String missing = this.getString(R.string.add_loan_payment_submit_missing);
        ...
        //TODO - 3/15/2017 - separate messages for negative interest rate and/or principal
        boolean interestRateEntered = !this.interestRateInput.getText().toString().equals("") && !(Double.parseDouble(interestRateInput.getText().toString()) < 0);
        boolean simpleInterestGood = !simpleInterestCheckbox.isChecked() ||
                ((simpleInterestCheckbox.isChecked() && !principalInput.getText().toString().equals("") && !(Double.parseDouble(principalInput.getText().toString()) < 0)));
        ...
        boolean missingBasic = !accountNameEntered || !currentValueEntered || !interestRateEntered || !simpleInterestGood ||
        ...
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_payment_debt_prompt);
        }
        if (!currentValueEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_payment_value_prompt);
        }
        if (!interestRateEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_payment_interest_rate_prompt);
        }
        if (!simpleInterestGood)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_simple_interest_principal);
        }

        if (!paymentFrequencyEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_payment_frequency_prompt);
        }
        if (!paymentAmountEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_payment_amount_prompt);
        }
        if (!sourceAccountSet)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_payment_source_accout_prompt);
        }
        if (!nextDateSet)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_loan_payment_next_prompt);
        }
        ...
        if (goalChecked)
        {
            ...
            if (!goalDateSet)
            {
                if (!missingBasic)
                {
                    errorToastString+=this.getString(R.string.add_loan_payment_submit_missing);
                }
                errorToastString+="\n";
                errorToastString+=this.getString(R.string.add_loan_payment_goal_date_prompt);
            }
            ...
            if (!missingBasic && goalDateSet)
            {
                ...
                if (!dataConsistent)
                {
                    errorToastString+=this.getString(R.string.add_loan_payment_submit_data_dirty);
                }
            }
        }
        else
        {
            ...
            if (!ongoing)
            {
                ...
                if (!endDateSet)
                {
                    ...
                    errorToastString+="\n";
                    errorToastString+=this.getString(R.string.add_loan_payment_end_prompt);
                }
            }
        }
        ...
        if (!editing && bbd.getDebtWithName(this.debtName.getText().toString()) != null)
        {
            if (errorToastString.equals("")) {
                errorToastString += this.getString(R.string.add_loan_payment_duplicate_debt_name);
            }
            else
            {
                errorToastString+="\n\n";
