...
/**
 * Add CreditCard Payment Activity - activity displays a form for setting up a payment for a debt. The
 * debt information is filled in with data already taken from the add credit card form,
 * but can still be edited. Heavily influenced by the Add Savings Activity
 *
 * This form is also used to edit an existing credit card to either add a new payment or edit an
 * existing one.
 */
public class AddCreditCardPayment extends BadBudgetChildActivity
        implements AdapterView.OnItemSelectedListener, DateInputActivity,
        CleanFragmentParent, BBOperationTaskCaller
{
    private static final String progressDialogMessage = "Adding Credit Card With Payment...";
    private static final String progressDialogMessageUpdate = "Updating Credit Card With Payment...";
    private static final String progressDialogMessageDelete = "Deleting Credit Card With Payment...";
    ...
    * where items are matched with keys in clean button method */
    ...
    private CreditCard editCreditCard;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContent(R.layout.content_add_credit_card_payment);
    ...
    private void setup()
    {
        debtName = (EditText) findViewById(R.id.addCreditCardPaymentDebtHint);
        debtAmount = (EditText) findViewById(R.id.addCreditCardPaymentCurrentValueHint);
        interestRate = (EditText) findViewById(R.id.addCreditCardPaymentInterestRateHint);
        quicklookCheckbox = (CheckBox) findViewById(R.id.addCreditCardPaymentQuicklookCheckbox);

        paymentAmount = (EditText) findViewById(R.id.addCreditCardPaymentAmountHint);
        payoffCheckbox = (CheckBox) findViewById(R.id.addCreditCardPaymentAmountPayoff);

        paymentFrequencySpinner = (Spinner) findViewById(R.id.addCreditCardPaymentFrequencySpinner);
        paymentSourceSpinner = (Spinner) findViewById(R.id.addCreditCardPaymentSourceSpinner);

        nextDateText = (TextView) findViewById(R.id.addCreditCardPaymentNextInput);

        endDateText = (TextView) findViewById(R.id.addCreditCardPaymentInputEnd);
        ongoingCheckbox = (CheckBox) findViewById(R.id.addCreditCardPaymentOngoingCheckbox);

        setGoalCheckbox = (CheckBox) findViewById(R.id.addCreditCardPaymentSetGoal);
        goalDateText = (TextView) findViewById(R.id.addCreditCardPaymentInputGoal);
        ...
        * to reference the accounts by using the name as a key */
        ...
        alreadyCleanToast = Toast.makeText(this, R.string.add_creditcard_payment_data_clean_message, Toast.LENGTH_SHORT);
        ...
        editCreditCard = (CreditCard)((BadBudgetApplication)this.getApplication()).getBadBudgetUserData().getDebtWithName(editName);
        editHasExistingPayment = args.getBoolean(BadBudgetApplication.EDIT_PAYMENT_KEY);

        if (editing)
        {
            this.setToolbarTitle(R.string.edit_creditcard_payment_title);
            debtName.setEnabled(false);
            debtName.setText(editCreditCard.name());
    ...
    private void firstTimeSetup()
    {
        ...
        if (editing && editHasExistingPayment)
        {
                ...
                //Fill out the basic debt info for the existing card with a payment
                //We came directly from the debts table
                debtAmount.setText(Double.toString(editCreditCard.amount()));
                interestRate.setText(Double.toString(editCreditCard.interestRate()));
                quicklookCheckbox.setChecked(editCreditCard.quicklook());
                Payment payment = editCreditCard.payment();
    ...
    * when setting up the form for the first time) */
    ...
    private void payoffChecked()
    {
        ...
        paymentAmount.setHint(R.string.add_creditcard_payment_amount_payoff);
    }
    ...
    private void payoffUnchecked()
    {
        ...
        paymentAmount.setHint(R.string.add_creditcard_payment_amount_hint);
    }
    ...
    private void ongoingChecked()
    {
        ...
        clearEndDate();
        endDateText.setHint(getString(R.string.add_creditcard_payment_ongoing));
    }
    ...
    private void ongoingUnchecked()
    {
        ...
        endDateText.setHint(R.string.add_creditcard_payment_date_hint);
    }
    ...
    private void disableEndDateInput()
    {
        ...
        endDateText.setClickable(false);
        clearEndDate();
        endDateText.setHint(R.string.add_creditcard_payment_end_date_disabled);
    }
    ...
    {
        dateClicked(AddCreditCardPayment.NEXT_DATE_SET_CODE, "nextDatePicker");
    }
    ...
    {
        dateClicked(AddCreditCardPayment.END_DATE_SET_CODE, "endDatePicker");
    }
    ...
    {
        dateClicked(AddCreditCardPayment.GOAL_DATE_SET_CODE, "goalDatePicker");
    }
     ...
     * @param code - the code identifying which date is being chosen (constants in AddCreditCardPaymentActivity)
     ...
     * @param view
     */
    public void addCreditCardPaymentCleanClick(View view)
    {
        ...
        boolean allEntered = currentValueEntered && interestRateEntered && paymentAmountEntered && goalCheckboxChecked && nextDateEntered && goalSet;
        ...
        if (allEntered)
        {
            ...
            if (!dataConsistent)
            {
                //Keep track of the correct goal date and payment amount  in case the user indicates this is what they want
                //to update.
                ...
                bundle.putString(CleanFragment.POSITION_ONE_TITLE_KEY, getString(R.string.add_creditcard_payment_goal_date_prompt));
                ...
                bundle.putString(CleanFragment.POSITION_TWO_TITLE_KEY, getString(R.string.add_creditcard_payment_amount_prompt));
        ...
        else if (missingOnlyPaymentAmount)
        {
            ...
            CharSequence text = getString(R.string.add_creditcard_payment_auto_set_payment_amount);
        ...
        else if (missingOnlyGoalDate)
        {
            ...
            CharSequence text = getString(R.string.add_creditcard_payment_auto_set_goal_date);
        ...
        {
            String goalCheckboxUnchecked = getString(R.string.add_creditcard_payment_data_goal_checkbox_unchecked);
        ...
        else
        {
            ...
            String missingMessage = getString(R.string.add_creditcard_payment_data_clean_missing);
            if (missingPaymentAndGoal)
            {
                missingMessage+="\n";
                missingMessage+= (getString(R.string.add_creditcard_payment_amount_prompt) + " " +
                        getString(R.string.add_creditcard_payment_data_clean_and_or) + " " +
                        getString(R.string.add_creditcard_payment_goal_date_prompt));
            }
            if (!currentValueEntered)
            {
                missingMessage+="\n";
                missingMessage+=getString(R.string.add_creditcard_payment_value_prompt);
            }
            if (!interestRateEntered)
            {
                missingMessage+="\n";
                missingMessage+=getString(R.string.add_creditcard_payment_interest_rate_prompt);
            }

            if (!nextDateEntered)
            {
                missingMessage+="\n";
                missingMessage+=getString(R.string.add_creditcard_payment_next_prompt);
            }
     ...
     * a new credit card object with a payment.
     * @param view
     */
    public void addCreditCardPaymentSubmitClick(View view)
    {
        if (verifyValues())
        {
            ...
            //Setup our credit card with a payment
            try
            {
                CreditCard creditCard = new CreditCard(name, debt, quicklook, interestRateDouble);
                //If the goal date is set then endDate is null so need to set what we pass for endDate to the goal date.
                if (goalDate != null)
                {
                    pEndDate = pGoalDate;
                }
                Payment payment = new Payment(pAmount, payoff, pFreq, pSource, pNextDate, pOngoing, pEndDate, creditCard, pGoalDate);
                creditCard.setupPayment(payment);
                ...
                if (editing)
                {
                    EditBBObjectTask task = new EditBBObjectTask(this, progressDialogMessage, creditCard, values, this, BBObjectType.DEBT_CREDITCARD);
                    task.execute();
                }
                else
                {
                    AddBBObjectTask task = new AddBBObjectTask(this, progressDialogMessageUpdate, creditCard, values, this, BBObjectType.DEBT_CREDITCARD);
                    task.execute();
                }
            }
     ...
     * @param view
     */
    public void addCreditCardPaymentCancelClick(View view)
    {
        this.onBackPressed();
    }
     ...
     * Kicks off a task to delete the creditCard we are editing from our bb data.
     * @param view
     */
    public void deleteClick(View view)
    {
        ...
        if (errorString == null) {
            DeleteBBObjectTask task = new DeleteBBObjectTask(this, progressDialogMessageDelete, editCreditCard.name(), this, BBObjectType.DEBT_CREDITCARD);
            task.execute();
        }
        else
        {
    ...
    /**
     * Callback indicating the invoked add task has completed. Sets result and finishes.
     */
    ...
    /**
     * Callback indicating the invoked edit task has completed. Sets result and finishes.
     */
    ...
    /**
     * Callback indicating the invoked delete task has completed. Sets result and finishes.
     */
     ...
     * a goal date. Returns a goal date consistent with all the values currently set or null if a
     * reasonable goal cannot be found
    ...
    private Date calculateGoalDate()
    {
        ...
        correctGoalDate = Prediction.findGoalDateCompoundInterest(this.nextDate, numInterestDays , paymentAmount, this.paymentFrequency, currentAmount, interestRate, limitDate);
    ...
    private boolean verifyValues()
    {
        ...
        String missing = this.getString(R.string.add_creditcard_payment_submit_missing);
        ...
        boolean interestRateEntered = !this.interestRate.getText().toString().equals("");
        ...
        boolean missingBasic = !accountNameEntered || !currentValueEntered || !interestRateEntered ||
        ...
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_creditcard_payment_debt_prompt);
        }
        if (!currentValueEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_creditcard_payment_value_prompt);
        }
        if (!interestRateEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_creditcard_payment_interest_rate_prompt);
        }
        if (!paymentFrequencyEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_creditcard_payment_frequency_prompt);
        }
        if (!paymentAmountEntered)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_creditcard_payment_amount_prompt);
        }
        if (!sourceAccountSet)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_creditcard_payment_source_accout_prompt);
        }
        if (!nextDateSet)
        {
            errorToastString+="\n";
            errorToastString+=this.getString(R.string.add_creditcard_payment_next_prompt);
        }
        ...
        if (goalChecked)
        {
            ...
            if (!goalDateSet)
            {
                if (!missingBasic)
                {
                    errorToastString+=this.getString(R.string.add_creditcard_payment_submit_missing);
                }
                errorToastString+="\n";
                errorToastString+=this.getString(R.string.add_creditcard_payment_goal_date_prompt);
            }
            ...
            if (!missingBasic && goalDateSet)
            {
                ...
                if (!dataConsistent)
                {
                    errorToastString+=this.getString(R.string.add_creditcard_payment_submit_data_dirty);
                }
            }
        }
        else
        {
            ...
            if (!ongoing)
            {
                ...
                if (!endDateSet)
                {
                    ...
                    errorToastString+="\n";
                    errorToastString+=this.getString(R.string.add_creditcard_payment_end_prompt);
                }
            }
        }
        ...
        if (!editing && bbd.getDebtWithName(this.debtName.getText().toString()) != null)
        {
            if (errorToastString.equals("")) {
                errorToastString += this.getString(R.string.add_creditcard_payment_duplicate_debt_name);
            }
            else
            {
                errorToastString+="\n\n";
    ...
    public String verifyCanDelete()
    {
        ...
        for (MoneyLoss loss : bbd.getLosses())
        {
            ...
            if (lossSource instanceof CreditCard && lossSource.name().equals(editCreditCard.name()))
            {
        ...
        if (budgetSource != null && budgetSource instanceof CreditCard && budgetSource.name().equals(editCreditCard.name()))
        {
